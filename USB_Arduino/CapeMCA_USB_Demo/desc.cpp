...
uint8_t getconfdescr( uint8_t addr, uint8_t conf, USB* pUsb );
...
uint8_t getdevdescr( uint8_t addr, uint8_t &num_conf, USB* pUsb );

void PrintDescriptors(uint8_t addr, USB* pUsb)
{
  uint8_t rcode = 0;
  uint8_t num_conf = 0;

  rcode = getdevdescr( (uint8_t)addr, num_conf, pUsb);
  if ( rcode )
  {
  ...
  {
    rcode = getconfdescr( addr, i, pUsb);                 // get configuration descriptor
    if ( rcode )
    {
      printProgStr(Gen_Error_str);
      print_hex(rcode, 8);
...
void PrintAllDescriptors(UsbDevice *pdev, USB* pUsb)
{
  Serial.println("\r\nAddr: ");
  print_hex(pdev->address.devAddress, 8);
  Serial.println("\r\n--");
  PrintDescriptors( pdev->address.devAddress, pUsb);
}
...
uint8_t getdevdescr( uint8_t addr, uint8_t &num_conf, USB* pUsb)
{
  ...
  uint8_t rcode;
  rcode = pUsb->getDevDescr( addr, 0, 0x12, ( uint8_t *)&buf );
  if ( rcode ) {
    return ( rcode );
  }
...
uint8_t getconfdescr( uint8_t addr, uint8_t conf, USB* pUsb)
{
  ...
  rcode = pUsb->getConfDescr( addr, 0, 4, conf, buf );  //get total length
